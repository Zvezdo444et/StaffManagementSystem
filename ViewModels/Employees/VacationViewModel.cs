...
public sealed class VacationViewModel : ObservableObject
{
    ...
    private VacationsDetailsViewModel? _detailsViewModel;
    public VacationsDetailsViewModel? DetailsViewModel
    {
    ...
    public VacationViewModel(IEmployeesService service)
    {
        _service = service;

        Selector = new EmployeeSelectorViewModel(service)
        {
            Subtitle = "Выберите работника для просмотра отпусков"
        };

        Selector.SelectEmployeeCommand = new RelayCommand<Employee>(OpenVacationData);
    }

    private async void OpenVacationData(Employee? employee)
    {
        ...
        DetailsViewModel = new VacationsDetailsViewModel(
            fullEmployee,
            () => IsDetailsOpen = false,
            OpenEditor
        );
