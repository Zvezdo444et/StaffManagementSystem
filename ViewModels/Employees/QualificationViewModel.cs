...
public sealed class QualificationViewModel : ObservableObject
{
    ...
    private QualificationDetailsViewModel? _detailsViewModel;
    public QualificationDetailsViewModel? DetailsViewModel
    {
    ...
    public QualificationViewModel(IEmployeesService service)
    {
        _service = service;
        Selector = new EmployeeSelectorViewModel(service)
        {
            Subtitle = "Выберите работника для просмотра данных по профподготовке"
        };
        Selector.SelectEmployeeCommand = new RelayCommand<Employee>(OpenQualificationData);
    }

    private async void OpenQualificationData(Employee? employee)
    {
        ...
        DetailsViewModel = new QualificationDetailsViewModel(
            fullEmployee,
            () => IsDetailsOpen = false,
            OpenEditor
        );
